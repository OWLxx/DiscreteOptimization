...
transport = m.addVars(warehouses, plants, obj=transCosts, name="trans")   # for 2 var, the obj is matrix
...
# is closed
m.addConstrs(
    (transport.sum('*',p) <= capacity[p]*open[p] for p in plants),  # left side is row
    "Capacity")
...
# Demand constraints
m.addConstrs(
    (transport.sum(w) == demand[w] for w in warehouses),  # warehouse is customoer here
    "Demand")
...
for p in plants:
    if open[p].x > 0.99:
        print('Plant %s open' % p, open[p].varName, open[p].x)
        for w in warehouses:
